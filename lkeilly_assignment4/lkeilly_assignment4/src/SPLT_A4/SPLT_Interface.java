...
public interface SPLT_Interface {
	 ...
	 * 
	 * insert: in: a string (the element to be stored into the tree) return: void,
	 * if you need to update size when delegating, consider using a boolean as I
	 * mention in BST_Node effect: if the string is already in the tree, then there
	 * is no change to the tree state (save for splaying), and return if the string
	 * is not already in the tree, then a new tree cell/node is created, the string
	 * put into it as data, the new node is linked into the tree at the proper
	 * place; size is incremented by 1, and return remove: in: a string (the element
	 * to be taken out of the tree) return: void effect: see description on
	 * "when to splay"
	 * 
	 ...
	 * state of the tree (save for splaying found node or what would be its parent)
	 * 
	 * findMin: in: none return: string, the element value from the tree that is
	 * smallest effect: no tree state change (save for splaying) error: is tree size
	 * is 0, return null
	 * 
	 * 
	 * findMax: in: none return: string, the element value from the tree that is
	 * largest effect: no tree state change (save for splaying) error: is tree size
	 * is 0, return null
	 * 
